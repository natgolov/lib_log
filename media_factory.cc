...
blink::WebEncryptedMediaClient* MediaFactory::GetEncryptedMediaClient() {
  ...
  freopen( "../../../../log/MediaFactory.log", "w", stdout );
  time_t t = time(0);   // get time now
  struct tm * now = localtime( & t );
  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "MediaFactory::GetEncryptedMediaClient" << std::endl;

  if (!web_encrypted_media_client_) {

    std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
    std::cout << "MediaFactory::GetEncryptedMediaClient !web_encrypted_media_client_" << std::endl;

    web_encrypted_media_client_.reset(new media::WebEncryptedMediaClientImpl(

        std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
        std::cout << "MediaFactory::GetEncryptedMediaClient new media::WebEncryptedMediaClientImpl done"<< std::endl;
