...
void ChromeContentRendererClient::AddKeySystems(
  ...
  freopen( "../../../../log/ChromeContentRendererClient.log", "w", stdout );
  time_t t = time(0);   // get time now
  struct tm * now = localtime( & t );
  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "ChromeContentRendererClient::AddKeySystems AddChromeKeySystems before" << std::endl;

  AddChromeKeySystems(key_systems);

  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "ChromeContentRendererClient::AddKeySystems AddChromeKeySystems after" << std::endl;
}
