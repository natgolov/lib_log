...
WebEncryptedMediaClientImpl::WebEncryptedMediaClientImpl(
  ...
  freopen( "../../../../log/WebEncryptedMediaClientImpl.log", "w", stdout );
  time_t t = time(0);   // get time now
  struct tm * now = localtime( & t );
  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "WebEncryptedMediaClientImpl::WebEncryptedMediaClientImpl KeySystems::GetInstance() done" << std::endl;
