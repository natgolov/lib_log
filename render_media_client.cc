...
void RenderMediaClient::AddSupportedKeySystems(
  ...
  freopen( "../../../../../log/RenderMediaClient.log", "w", stdout );
  time_t t = time(0);   // get time now
  struct tm * now = localtime( & t );
  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "RenderMediaClient::AddSupportedKeySystems AddKeySystems before" << std::endl;

  GetContentClient()->renderer()->AddKeySystems(key_systems_info);

  std::cout << t << ' ' << now->tm_hour << ':' << now->tm_min << ':' << now->tm_sec << ' ';
  std::cout << "RenderMediaClient::AddSupportedKeySystems AddKeySystems after" << std::endl;
